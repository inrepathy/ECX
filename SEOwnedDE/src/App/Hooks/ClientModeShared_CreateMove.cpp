...
#include "../Features/AntiAim/AntiAim.h"  // Include your AntiAim header
	...
	CClientModeShared* ecx, float flInputSampleTime, CUserCmd* pCmd)
{
	...
	F::Misc->CircleStrafer(pCmd);
	F::Misc->FastStop(pCmd);
    F::Misc->DuckInAir(pCmd);
	F::Misc->NoiseMakerSpam();
	F::Misc->AutoEdgebug(pCmd);
	F::Misc->AutoRocketJump(pCmd);
	F::Misc->LegJitter(pCmd);
	F::Misc->SequenceFreeze(pCmd, pSendPacket);
	F::Misc->AntiAfk(pCmd);
	F::Misc->RageRetry();
	...
	if (CFG::Exploits_Warp_Exploit && CFG::Exploits_Warp_Mode == 1 && Shifting::bShiftingWarp)
	{
		if (auto pLocal = H::Entities->GetLocal())
		{
			...
			if (CFG::Exploits_Warp_Exploit == 3) 
			{
				if (Shifting::nAvailableTicks <= (MAX_COMMANDS / 2)) 
				{
					Vec3 vAngle = {};
					Math::VectorAngles(pLocal->m_vecVelocity(), vAngle);
					pCmd->viewangles.x = 75.0f; 
					pCmd->viewangles.y = vAngle.y + 45.0f; 
					*pSendPacket = false;
					G::bSilentAngles = true;
					pCmd->sidemove = pCmd->forwardmove = 0;
				}
