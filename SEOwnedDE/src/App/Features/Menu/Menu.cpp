...
bool CMenu::ColorPicker(const char *szLabel, Color_t &colVar)
{
	...
	if (m_mapStates[&colVar])
	{
		...
		H::LateRender->OutlinedRect(x - 1, y - 1, w + 2, h + 2, Color_t(0, 0, 0, 255));
	}
...
void CMenu::MainWindow()
{
	...
	{
		enum class EAimTabs { AIMBOT, TRIGGERBOT, HVH};
		...
		if (Button("Automations", AimTab == EAimTabs::TRIGGERBOT))
		...
		if (AimTab == EAimTabs::AIMBOT)
		{
			...
			GroupBoxStart("Global", 150);
			{
				...
				multiselect("Accuracy", Accuracy, {
					{ "Accurate Unlag", CFG::Lagfix },
					{ "Hitscan Backtrack", CFG::Aimbot_Hitscan_Target_LagRecords},
					{ "Melee Backtrack", CFG::Aimbot_Melee_Target_LagRecords }
					});
		...
		if (AimTab == EAimTabs::HVH)
		{
			...
			GroupBoxStart("Global", 150);
			{
				...
				CheckBox("Resolver", CFG::Resolver);
				if (CFG::Resolver) {

					multiselect("Resolver Options", Resolver, {
						{ "Resolve Pitch", CFG::Resolver_Pitch },
						{ "Resolve Yaw", CFG::Resolver_Yaw },
						{ "Clamp", CFG::ResolverClamp },
						});
				}
...
{ "Disable if FD", CFG::Fakelag_DisableifFD },
					...
					SliderInt("Fakelag Ticks", CFG::Fakelag_Ticks, 1, 22, 1);
	...
	{
		enum class EVisualsTabs { ESP, RADAR, MATERIALS, OUTLINES, OTHER, OTHER2 };
	...
	/*	if (VisualsTab == EVisualsTabs::ESP) {
			...
			I::MatSystemSurface->DrawSetColor(40, 40, 40, 255);  
			I::MatSystemSurface->DrawFilledRect(rectX, rectY, rectX + rectWidth, rectY + rectHeight);

			I::MatSystemSurface->DrawSetColor(255, 255, 255, 255); 
			I::MatSystemSurface->DrawOutlinedRect(rectX, rectY, rectX + rectWidth, rectY + rectHeight);

			HFont nameFont = I::MatSystemSurface->CreateFont();
			I::MatSystemSurface->SetFontGlyphSet(nameFont, "Arial", 16, 400, 0, 0, FONTFLAG_ANTIALIAS);
			I::MatSystemSurface->DrawSetTextFont(nameFont);
			I::MatSystemSurface->DrawSetTextColor(255, 255, 255, 255); 
			I::MatSystemSurface->DrawSetTextPos(rectX + (rectWidth / 2) - 30, rectY + 10);  
			const wchar_t* playerName = L"PlayerName";
			I::MatSystemSurface->DrawPrintText(playerName, wcslen(playerName));

			int healthBarX = rectX + 10;
			int healthBarY = rectY + 50;
			int healthBarWidth = 4;  
			int healthBarHeight = 40;
			int healthPercent = 75;  

			I::MatSystemSurface->DrawSetColor(80, 80, 80, 255);  
			I::MatSystemSurface->DrawFilledRect(healthBarX, healthBarY, healthBarX + healthBarWidth, healthBarY + healthBarHeight);

			int healthHeight = (healthPercent * healthBarHeight) / 100;
			I::MatSystemSurface->DrawSetColor(0, 255, 0, 255);  
			I::MatSystemSurface->DrawFilledRect(healthBarX, healthBarY + (healthBarHeight - healthHeight), healthBarX + healthBarWidth, healthBarY + healthBarHeight);

			I::MatSystemSurface->DrawSetColor(255, 255, 255, 255);  
			I::MatSystemSurface->DrawOutlinedRect(healthBarX, healthBarY, healthBarX + healthBarWidth, healthBarY + healthBarHeight);
		}*/
		...
		if (VisualsTab == EVisualsTabs::ESP)
		{
			...
			GroupBoxStart("Global", 150);
			{
				...
				CheckBox("Use Custom Health Color", CFG::UseCustomHealthColor);
				if (CFG::UseCustomHealthColor) {
					ColorPicker("Custom Health Color", CFG::CustomHealthColor);
					CheckBox("Gradient", CFG::UseCustomHealthGradient);
					if (CFG::UseCustomHealthGradient) {
					//	CheckBox("Gradient", CFG::UseCustomHealthGradient); // if this is UseCustomHealthGradient and i use if (CFG::UseCustomHealthGradient) for it to turn on you can never turn it on...
						ColorPicker("Health Top", CFG::CustomHealthGradientStart);
						ColorPicker("Health Bottom", CFG::CustomHealthGradientEnd);
					}
		...
		if (VisualsTab == EVisualsTabs::OTHER)
		{
			...
			{
				CheckBox("Watermark", CFG::Watermark);
				CheckBox("Indicators", CFG::Indicators);
				...
				ColorPicker("Movement Path Color", CFG::MovementPath_Color);
				CheckBox("End Color Gradient", CFG::LongColor_Enabled);
				if (CFG::LongColor_Enabled) {
					ColorPicker("End Move Path Color", CFG::LongPath_Color);
				}
			...
			GroupBoxStart("Binds List", 150);
			{
				CheckBox("Active", CFG::BindsList_Active);
				SliderFloat("Outline Alpha", CFG::BindsList_Outline_Alpha, 0.1f, 1.0f, 0.1f, "%.1f");
				SliderFloat("Background Alpha", CFG::BindsList_Background_Alpha, 0.1f, 1.0f, 0.1f, "%.1f");
				SliderInt("Width", CFG::BindsList_Width, 200, 1000, 1);
			}
			GroupBoxEnd();
		}
	...
	if (MainTab == EMainTabs::MISC)
	{
		...
		GroupBoxStart("Misc", 160);
		{
			...
			CheckBox("Fast Acceleration", CFG::FastAccel);
		...
		GroupBoxStart("Shifting", 150);
		{
			...
			CheckBox("Desync", CFG::SequenceFreeze);
			InputKey("Desync", CFG::SequenceFreezeKey);
		...
		GroupBoxStart("Crits", 150);
		{
			...
			CheckBox("Draw Indicator", CFG::CritIndicator);
		}
		GroupBoxEnd();
